...
\ac{ECB}-encryption disappears with the right Block Cipher Mode of Operation.
...
and the password. The result is \ac{XOR}ed with 128 bits of plaintext to
...
\ac{XOR}ed with an unique combination of IV, nonce and key. This makes it
...
The present implementation is required to use \ac{ECB}, since it has to be
...
\caption{\ac{ECB}: Every block of plaintext is simply encrypted with a key.\cite{wikiecb}}
\end{figure}

\begin{figure}
\centering
\includegraphics[scale = 0.25]{data/figures/CTR_encryption.png}
\caption{CTR: Every block of plaintext is \ac{XOR}ed with a combination of initialization vector (here: Nonce) and counter, encrypted by the key to form the ciphertext. \cite{wikictr}
\end{figure}
